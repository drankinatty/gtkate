 ...
 *
 *  TODO - verify operation - changed from gtkwrite - no inst->language or
 *  inst->langname (may need to add, but intended to just call sourceview
 *  funcitons to obtain from lang_id when required)
 *  DONE - modified sourceview_buffer_set_language() below to use lang.
 */
...
void hghltmenu_set_none (GtkMenuItem *menuitem, gpointer data)
{
    ...
    gtk_source_buffer_set_language (inst->buf, NULL);
...
void sourceview_buffer_set_language (const gchar *name, gpointer data)
{
    ...
    if ((lang = sourcelanguage_lookup (name, data)))
        gtk_source_buffer_set_language (inst->buf, lang);
}
...
void addsubmenu (gconstpointer m, gpointer data)
{
    ...
    g_ptr_array_foreach (lm, (GFunc) addlangmenu, data);
 ...
 *  returns submenu, combobox is preserved as part of data structure.
 */
...
void sourceview_set_comment_syntax (gpointer data)
{
    ...
    if (!inst->language) {
        g_warning ("sourceview_set_comment_syntax() - languange not set");
        return;
    }
    ...
    if (!lang_id) {
        g_warning ("sourceview_set_comment_syntax() - lang_id not found");
        return;
    }

    while (cmtsyntax[i].id) {
        const gchar *p1 = cmtsyntax[i].id,  /* pointer to comment id */
                    *p2 = lang_id;          /* poiner to inst->lang_id */
        while (*p1 && *p2 && *p1 == *p2)    /* string compare */
            p1++, p2++;
        if (!*p1 && !*p2) { /* if both pointer point to nul-char */
            inst->comment_single = cmtsyntax[i].single;
            inst->comment_blk_beg = cmtsyntax[i].beg;
            inst->comment_blk_end = cmtsyntax[i].end;
            return;
        }
        i++;
    }
...
void sourceview_guess_language (gpointer data)
{
    ...
    if (inst->language) /* prevent changing manually applied language */
        return;
    ...
    content_type = g_content_type_guess (inst->filename, NULL,
                                        0, &result_uncertain);
    if (result_uncertain)
    {
        g_free (content_type);
    ...
    inst->language = gtk_source_language_manager_guess_language (app->langmgr,
                                                inst->filename, content_type);
    gtk_source_buffer_set_language (inst->buf, inst->language);

    // if (app->language)
    //     app->langname = gtk_source_language_get_name (app->language);

    sourceview_set_comment_syntax (data);

    if (app->laststyle)
        sourceview_set_syntax_laststyle (data);

    gtk_source_buffer_set_highlight_syntax (inst->buf, app->highlight);

    g_free (content_type);

}

void sourceview_get_languange_info (gpointer data)
{
    mainwin_t *app = data;
    kinst_t *inst = app->einst[app->focused]->inst;
    ...
    style_id   = gtk_source_language_get_id (inst->language);
    name       = gtk_source_language_get_name (inst->language);
    section    = gtk_source_language_get_section (inst->language);
    metadata   = gtk_source_language_get_metadata (inst->language, name);
    style_name = gtk_source_language_get_style_name (inst->language, style_id);
    ...
    /* get mime_types */
    mime_types = gtk_source_language_get_mime_types (inst->language);
    ...
    /* get globs */
    globs = gtk_source_language_get_globs (inst->language);
