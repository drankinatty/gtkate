                                  ...
                                  gpointer data)
{
    switch (event->keyval)
    {
        ...
        case GDK_KP_Enter:  /* toggle focus to set value in spinbtn */
            ...
            gtk_widget_child_focus (widget, GTK_DIR_LEFT);
            goto_btnfind (widget, data);
            return TRUE;
        case GDK_Return:
            gtk_widget_child_focus (widget, GTK_DIR_RIGHT);
            gtk_widget_child_focus (widget, GTK_DIR_LEFT);
...
GtkWidget *create_goto_dlg (gpointer data)
{
    ...
    GtkWidget *vscale;
    GtkTextIter iter;
    GtkTextMark *imark = gtk_text_buffer_get_insert (GTK_TEXT_BUFFER(inst->buf));

    gtk_text_buffer_get_iter_at_mark (GTK_TEXT_BUFFER(inst->buf), &iter, imark);
    ...
    if (last == 1) {
        // show_info_bar_markup_ok ("insufficient number of lines in buffer "
        //                         "(nowhere to go to).",
    ...
    gtk_widget_set_sensitive (btnfind, (line !=
